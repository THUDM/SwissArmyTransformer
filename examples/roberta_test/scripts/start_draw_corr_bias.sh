...
gpt_options=" \
       --experiment-name finetune-$MODEL_TYPE-${dataset_name}-onlym2-1e-4-\
...
run_cmd="${OPTIONS_NCCL} deepspeed --include=localhost:$FINETUNE_GPU --master_port ${port} --hostfile ${HOST_FILE_PATH} ex_dir/draw_corr_bias.py ${gpt_options}"
echo ${run_cmd}
eval ${run_cmd}
