...
MODEL_TYPE="swiss-cait-s24-224"
if [ ! -d "${CHECKPOINT_PATH}/$MODEL_TYPE" ]
then
    if [ ! -f "${CHECKPOINT_PATH}/$MODEL_TYPE.zip" ]
    then
        wget "https://cloud.tsinghua.edu.cn/f/30749ae3b424445ea44c/?dl=1" -O "${CHECKPOINT_PATH}/$MODEL_TYPE.zip"
    fi
...
run_cmd="${OPTIONS_NCCL} deepspeed --master_port 16666 --num_nodes ${NUM_WORKERS} --num_gpus ${NUM_GPUS_PER_WORKER} --hostfile ${HOST_FILE_PATH} finetune_cait_cifar10.py ${gpt_options}"
echo ${run_cmd}
eval ${run_cmd}
