...
def get_batch(data_iterator, args, timers):
    ...
    dec_position_ids = torch.zeros(1, device=image_data.device, dtype=torch.long)
    torch.arange(0, 1, out=dec_position_ids[:1])
    dec_position_ids = dec_position_ids.unsqueeze(0).expand([batch_size, -1])
    
    tokens = torch.zeros((batch_size, 1), device=image_data.device, dtype=torch.long)
    # Convert
    if args.fp16:
        image_data = image_data.half()
    return tokens, image_data, label_data, dec_position_ids, position_ids
...
def forward_step(data_iterator, model, args, timers):
    ...
    tokens, images, labels, dec_position_ids, position_ids = get_batch(
        data_iterator, args, timers)

    timers('batch generator').stop()

    encoder_outputs, decoder_outputs, *mems = model(tokens, position_ids, dec_position_ids, image=images, offline=True) #, offline=False, height=384//16, width=384//16)
...
if __name__ == '__main__':
    ...
    model = CaiTFinetuneModel.from_pretrained(py_parser)
    if args.fp16:
        model.half()
    elif args.bf16:
        model.bfloat16()
