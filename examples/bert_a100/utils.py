...
tokenizer = BertTokenizer.from_pretrained(os.path.join(pretrain_path, 'bert-large-uncased'))
tokenizers={}
tokenizers['bert-large-uncased'] = BertTokenizer.from_pretrained(os.path.join(pretrain_path, 'bert-large-uncased'),local_files_only=True)
tokenizers['bert-large-uncased-addprefix'] = BertTokenizer.from_pretrained(os.path.join(pretrain_path, 'bert-large-uncased'),local_files_only=True, add_prefix_space=True)
from SwissArmyTransformer.data_utils import load_hf_dataset


def get_dataset_keys(dataset_name):
    dataset_keys = {
        #superglue has changed boolq, copa, cb,
        'boolq':["input_ids", "position_ids","token_type_ids", "attention_mask", "label"],
        'rte':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        'copa':["input_ids_1", "position_ids_1", 'token_type_ids_1',"attention_mask_1", "input_ids_2", "position_ids_2", 'token_type_ids_2', "attention_mask_2", "label"],
        'cb':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        'wic':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label", "pos1", "pos2", "pos1_e", "pos2_e"],
        'multirc': ["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        'wsc': ["label", "query_tokens", "query_mask", "cand_tokens", "cand_mask"],
        #glue
        'mrpc':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        'qnli':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        'qqp':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        'cola':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        'wnli':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        'sst2': ["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        'stsb': ["input_ids", "position_ids", "token_type_ids", "attention_mask", "stsb_label"],
        #squad
        'squad':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label", "start_list", "end_list"],
        'squad_v2':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label", "start_list", "end_list"],
        #conll
        'conll2003':["input_ids", "position_ids", "token_type_ids", "attention_mask", "label"],
        # emotion
...
def get_batch_function(dataset_name):
    if dataset_name == "stsb" :
        def get_batch(data_iterator, args, timers):
            ...
            keys = ['input_ids', 'position_ids', "token_type_ids", 'attention_mask', 'stsb_label']
    ...
    elif dataset_name in ["boolq", "rte", "cb", "mrpc", "qnli", "qqp", "cola", 'wnli', 'conll2003', 'sst2', 'emotion', 'multirc'] :
        def get_batch(data_iterator, args, timers):
            # Items and their type.
        ...
        def get_batch(data_iterator, args, timers):
            # Items and their type.
            keys = ["input_ids_1", "position_ids_1", 'token_type_ids_1', "attention_mask_1", "input_ids_2", "position_ids_2", 'token_type_ids_2', "attention_mask_2", "label"]
            datatype = torch.int64
            ...
            tokens_ids_1 = data_b['token_type_ids_1'].long()
            tokens_ids_2 = data_b['token_type_ids_2'].long()
            tokens_ids = torch.cat([tokens_ids_1, tokens_ids_2], dim=0)
            ...
            return tokens, labels, attention_mask, position_ids, tokens_ids, (tokens!=1)
    elif dataset_name == 'wic':
        def get_batch(data_iterator, args, timers):
            # Items and their type.
            keys = ['input_ids', 'position_ids', 'token_type_ids', 'attention_mask', 'label', 'pos1', 'pos2', "pos1_e", "pos2_e"]
            datatype = torch.int64
            ...
            bz = labels.shape[0]
            word1 = torch.zeros_like(tokens, dtype=attention_mask.dtype,
                                     device=tokens.device)
            word2 = torch.zeros_like(tokens, dtype=attention_mask.dtype,
                                     device=tokens.device)
            for i in range(bz):
            ...
            if args.fp16:
                ...
                word1 = word1.half()
                word2 = word2.half()
            return tokens, labels, attention_mask, position_ids, token_type_ids, (tokens != 1), {'pos1': pos1, 'pos2': pos2, 'word1': word1, 'word2': word2}

    elif dataset_name in ['squad', 'squad_v2']:
        def get_batch(data_iterator, args, timers):
            # Items and their type.
            keys = ['input_ids', 'position_ids', 'token_type_ids', 'attention_mask', 'label', 'start_list', 'end_list']
            datatype = torch.int64
            ...
            return tokens, labels, attention_mask, position_ids, 'token_type_ids', (tokens!=1), {'start_list':start_list, 'end_list':end_list}
    else:
...
def _encode_single_text(text, args, is_split_into_words=False):
    ...
    encoded_input = tokenizers[tokenizer_name](text, max_length=args.sample_length, padding='max_length', truncation='only_first',
                              is_split_into_words=is_split_into_words)
    #position_ids = create_position_ids_from_input_ids(torch.tensor([encoded_input['input_ids']]), 1, 0)
    position_ids = torch.arange(len(encoded_input['input_ids']))
    return dict(input_ids=encoded_input['input_ids'], position_ids=position_ids.numpy(),
                token_type_ids=encoded_input['token_type_ids'], attention_mask=encoded_input['attention_mask'])
...
def _encode_double_text(text, text_pair, args, truncation='only_first', is_split_into_words=False):
    ...
    encoded_input = tokenizers[tokenizer_name](text, text_pair,
                                               max_length=args.sample_length, padding='max_length',
                                               truncation=truncation, is_split_into_words=is_split_into_words)
...
def create_dataset_function(path, args):
    ...
    offline = False     # 使用本地数据库
    transformer_name = f"bert_{dataset_name}_transformer_{args.sample_length}"
    process_fn = None
    filter_fn = None
    ...
    if dataset_name == "stsb":
        ...
        def process_fn(row):
            ...
            pos1 = tokenizers[args.name_model + "-addprefix"](row['sentence1'][:start1], is_split_into_words=split)[
                       'input_ids'].__len__() - 2
            pos1_e = tokenizers[args.name_model + "-addprefix"](row['sentence1'][:end1], is_split_into_words=split)['input_ids'].__len__() - 2
            if pos1 == 0:
                pos1 += 1
            pos2 = tokenizers[args.name_model + "-addprefix"](row['sentence2'][:start2], is_split_into_words=split)[
                   'input_ids'].__len__() - 2
        ...
        def process_fn(row):
            ...
            return {
                ...
                'token_type_ids_1': np.array(pack_1['token_type_ids'], dtype=np.int64),
                'token_type_ids_2': np.array(pack_2['token_type_ids'], dtype=np.int64),
        ...
        def process_fn(row):
            ...
            pad = lambda a: a[0:args.sample_length] if len(a) > args.sample_length else a + [1] * (
                        args.sample_length - len(a))
            text = row['text']
