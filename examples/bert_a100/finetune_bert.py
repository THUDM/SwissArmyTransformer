...
from utils import *
from bert_model import MLP_bert_Mixin

class ClassificationModel(BertModel):
    def __init__(self, args, transformer=None, parallel_output=True):
        super().__init__(args, transformer=transformer, parallel_output=parallel_output)
        self.del_mixin('final_forward')
        num_input = args.final_input
        #num_input = 1024
        num_output = 1 if args.class_num == 2 else args.class_num
        #self.add_mixin('classification_head', MLP_bert_Mixin(num_input, 2048, num_output))
        layer_range = None
...
def forward_step(data_iterator, model, args, timers):
    ...
    tokens, labels, attention_mask, position_ids, token_type_ids, loss_mask, *extra_data = get_batch(
    ...
    if len(extra_data) >= 1:
        ...
        extra_data = {}
    attention_output = []
    logits, *mems = model(tokens, position_ids,  attention_mask, token_type_ids=token_type_ids, attention_output = attention_output, **extra_data)
